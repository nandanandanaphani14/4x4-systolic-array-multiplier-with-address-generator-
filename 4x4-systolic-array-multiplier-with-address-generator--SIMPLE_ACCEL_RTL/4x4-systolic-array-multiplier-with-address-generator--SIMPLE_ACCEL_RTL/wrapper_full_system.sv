...
//
//---------------------------------------------------------------------
// Weight reuse is automatic
//---------------------------------------------------------------------
//   Step 2 is optional for the weight half.  If you do not write
//   weight_mem between two jobs, this module notices and tells the FSM
//   to skip WLOAD entirely, because the array still physically holds
//   the previous matrix.  Nothing in the host protocol changes; the
//   job simply finishes ROWS cycles sooner.
//
//     job with a new weight matrix   :  ROWS + N + STORE_LATENCY + 1
//                                    =  N + 13   (17 clocks for N = 4)
//     job reusing the loaded weights :  N + STORE_LATENCY + 1
//                                    =  N +  9   (13 clocks for N = 4)
//
//   Write any weight row - even the same value - and the next job
//   reloads.  The flag is conservative in the safe direction.
//=====================================================================
module accelerator_system_top #(
    ...
    // Control Unit (FSM)
    //-----------------------------------------------------------------
    logic wload_phase, compute_phase, drain_phase;
    logic weights_dirty;
    ...
    //-----------------------------------------------------------------
    // Weight-reuse tracking  ->  controller.load_weights
    //-----------------------------------------------------------------
    // The array is weight-stationary in the strongest sense: MAC.sv
    // writes weight_q only while wload is high, so the loaded matrix
    // survives COMPUTE, DRAIN, DONE and IDLE untouched.  Re-shifting it
    // for a job that uses the same weights is therefore pure overhead,
    // and skipping WLOAD saves ROWS cycles per job.
    //
    // weights_dirty means "the array does not hold what weight_mem
    // contains".  It is:
    //   SET    by any host weight write that actually reaches the SRAM
    //          (host writes are masked while busy, so gate on !busy),
    //   CLEARED on the last WLOAD cycle, once the new matrix is in,
    //   SET    by reset, because every weight_q comes up zero and the
    //          first job after reset must load.
    //
    // Deliberately internal: this module's port list is unchanged, so
    // every existing instantiation keeps working and no host has to
    // learn a new command bit.  The AGU counts w_addr DOWN ROWS-1..0,
    // so w_addr = 0 marks the final WLOAD cycle.
    //-----------------------------------------------------------------
    logic wload_last;
    assign wload_last = wload_phase && (agu_w_addr == '0);

    always_ff @(posedge clk) begin
        if (!rst_n)                     weights_dirty <= 1'b1;
        else if (host_w_we && !busy)    weights_dirty <= 1'b1;
        else if (wload_last)            weights_dirty <= 1'b0;
    end
